using Godot;

namespace DownfallU.DownfallUCode.Extensions;

//Mostly utilities to get asset paths.
public static class StringExtensions
{
    public static string ImagePath(this string path, string characterId)
    {
        return Path.Join(MainFile.ResPath, "images", characterId.ToLowerInvariant(), path);
    }

    public static string EnchantmentImagePath(this string path)
    {
        path = Path.Join(MainFile.ResPath, "images", "enchantments", path);
        if (ResourceLoader.Exists(path)) return path;

        MainFile.Logger.Info("Could not find enchantment image path: " + path);
        return Path.Join(MainFile.ResPath, "images", "enchantments", "enchantment.png");
    }
    public static string EventImagePath(this string path, string characterId)
    {
        path = Path.Join(MainFile.ResPath, "images", "events", characterId.ToLowerInvariant(), path);
        if (ResourceLoader.Exists(path)) return path;

        MainFile.Logger.Info("Could not find event image path: " + path);
        return Path.Join(MainFile.ResPath, "images", "events", characterId.ToLowerInvariant(), "event.png");
    }

    public static string CardImagePath(this string path, string characterId)
    {
        path = Path.Join(MainFile.ResPath, "images", "card_portraits", characterId.ToLowerInvariant(), path);
        if (ResourceLoader.Exists(path)) return path;
        
        MainFile.Logger.Info("Could not find card image path: " + path);
        return Path.Join(MainFile.ResPath, "images", "card_portraits", "card.png");
    }

    public static string BigCardImagePath(this string path, string characterId)
    {
        path = Path.Join(MainFile.ResPath, "images", "card_portraits", characterId.ToLowerInvariant(), "big", path);
        if (ResourceLoader.Exists(path)) return path;
        
        MainFile.Logger.Info("Could not find big card image path: " + path);
        return Path.Join(MainFile.ResPath, "images", "card_portraits", "big", "card.png");
    }

    public static string PowerImagePath(this string path, string characterId)
    {
        path = Path.Join(MainFile.ResPath, "images", "powers", characterId.ToLowerInvariant(), path);
        if (ResourceLoader.Exists(path)) return path;
        
        MainFile.Logger.Info("Could not find power image path: " + path);
        return Path.Join(MainFile.ResPath, "images", "powers", "power.png");
    }

    public static string BigPowerImagePath(this string path, string characterId)
    {
        path = Path.Join(MainFile.ResPath, "images", "powers", characterId.ToLowerInvariant(), "big", path);
        if (ResourceLoader.Exists(path)) return path;
        
        MainFile.Logger.Info("Could not find big power image path: " + path);
        return Path.Join(MainFile.ResPath, "images", "powers", "big", "power.png");
    }

    public static string RelicImagePath(this string path, string characterId)
    {
        path = Path.Join(MainFile.ResPath, "images", "relics", characterId.ToLowerInvariant(), path);
        if (ResourceLoader.Exists(path)) return path;
        
        MainFile.Logger.Info("Could not find relic image path: " + path);
        return Path.Join(MainFile.ResPath, "images", "relics", "relic.png");
    }

    public static string BigRelicImagePath(this string path, string characterId)
    {
        path = Path.Join(MainFile.ResPath, "images", "relics", characterId.ToLowerInvariant(), "big", path);
        if (ResourceLoader.Exists(path)) return path;
        
        MainFile.Logger.Info("Could not find big relic image path: " + path);
        return Path.Join(MainFile.ResPath, "images", "relics", "big", "relic.png");
    }
    
    public static string PotionImagePath(this string path, string characterId)
    {
        path = Path.Join(MainFile.ResPath, "images", "potions", characterId.ToLowerInvariant(), path);
        if (ResourceLoader.Exists(path)) return path;

        MainFile.Logger.Info("Could not find potion image path: " + path);
        return Path.Join(MainFile.ResPath, "images", "potions", "potion.png");
    }

    public static string CharacterUiPath(this string path, string characterId)
    {
        return Path.Join(MainFile.ResPath, "images", "charui", characterId.ToLowerInvariant(), path);
    }
}